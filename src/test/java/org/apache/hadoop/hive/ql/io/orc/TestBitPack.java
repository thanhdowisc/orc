...
import org.hortonworks.compression.ORCIntegerEncoding;
...
public class TestBitPack {
	...
	private void runTest(int numBits) {
		...
		fixedWidth = ORCIntegerEncoding.FixedBitSizes.getClosestFixedBits(fixedWidth);
		byte[] packed = BitPackWriter.pack(deltaEncoded, deltaEncoded.length, fixedWidth);
		long[] deltaDec = BitPackReader.unpack(packed, fixedWidth, deltaEncoded.length);
	...
	public void test01BitPacking1Bit() {
		...
		fixedWidth = ORCIntegerEncoding.FixedBitSizes.getClosestFixedBits(fixedWidth);
		byte[] packed = BitPackWriter.pack(deltaEncoded, deltaEncoded.length, fixedWidth);
		long[] deltaDec = BitPackReader.unpack(packed, fixedWidth, SIZE);
 		long[] out = new long[deltaDec.length];
 		for(int i=0; i<deltaDec.length; i++) {
 			out[i] = Utils.zigzagDecode(deltaDec[i]);
 		}
