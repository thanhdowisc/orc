...
/**
 * A red-black tree that stores doubles. The doubles are stored as the integer
 * representation.
 */
class DoubleRedBlackTree extends RedBlackTree {
  private final DynamicDoubleArray keys = new DynamicDoubleArray();
  ...
  protected int compareValue(int position) {
    double other = keys.get(position);
    if (newKey > other) {
      return 1;
    } else {
  ...
  private class VisitorContextImpl implements VisitorContext {
    ...
    public int getCount() {
      return DoubleRedBlackTree.this.getCount(originalPosition);
    }
  }
