...
abstract class RedBlackTree
{
   ...
   */
  public RedBlackTree(int initial_capacity) {
    data = new DynamicIntArray(initial_capacity * ELEMENT_SIZE);
  }
   ...
   */
  private void setRed(int position, boolean is_red) {
   ...
   * @param great_grandparent Grandparent's parent
  ...
  private boolean add(int node, boolean fromLeft, int parent,
    ...
    if (node == NULL) {
      ...
      if (compare < 0) {
        keep_going = add(getLeft(node), true, node, parent, grandparent);
      } else if (compare > 0) {
        keep_going = add(getRight(node), false, node, parent, grandparent);
      } else {
        lastAdd = node;
        wasAdd = false;
      ...
      if (parent == getLeft(grandparent)) {
        ...
        if (isRed(uncle)) {
          ...
          if (great_grandparent == NULL) {
            root = parent;
          } else if (getLeft(great_grandparent) == grandparent) {
            setLeft(great_grandparent, parent);
          } else {
            setRight(great_grandparent, parent);
          }
        ...
        if (isRed(uncle)) {
          ...
          if (great_grandparent == NULL) {
            root = parent;
          } else if (getRight(great_grandparent) == grandparent) {
            setRight(great_grandparent, parent);
          } else {
            setLeft(great_grandparent, parent);
          }
